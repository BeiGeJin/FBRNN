...
\section*{Week 06/26 - 07/10}

\subsection*{Main Points}

\noindent
\begin{itemize}
    \item Add the sparse matrix for inputs.
    \item Add the rescale mechanism after the output neuron and make the feedback control insensitive to the scale.
    \item Try a new mechanism to turn the feedback control off manually.
\end{itemize}

\newpage

\subsection*{The Current Pipeline for Learning Readout Weights}

\noindent
\textbf{RNN Basic Setup}:

One recurrent layer, with each node getting external input, fully connected to one output node by readout weight matrix, following Dale's law.
...
\begin{itemize}
    \item number of nodes: 200
    \item recurrent weight matrix: random
    \item excitatory nodes proportion: 50\%
    \item initial gains (recurrent layer): 1
    \item initial shifts (recurrent layer): 0
    \item initial gain (output node): 1
    \item initial shift (output node): 0
\end{itemize}

\noindent
\textbf{External Inputs}:

A basis set of sine waves with random periods and phases, connecting to neurons through a normalized sparse matrix.

\begin{itemize}
    \item number of wave basis: 500
    \item periods range: [8, 12] 
    \item phases range: [0, max phase given period]
    \item density of the sparse matrix: 10/500
\end{itemize}

\noindent
\textbf{Feedback Control}:

Linear approximation the dynamic system at the current time point: $$x(t+1) = f(x(t), u_g(t), u_s(t)) \approx  Fx(t) + G_gu_g(t) + G_su_s(t) + \text{const} \approx  F_{aug}x(t) + G_{aug}u(t)$$
Then using LQR (discreate-time tracking) on both gains and shifts. \textcolor{red}{LQR only cares how to correct the period and phase, invariant to the scale of current output.}
...
\begin{itemize}
    \item rule: Hebbian Covariance Rule (modified version from Kris)
    \item positive learning rate: 0.05
    \item negative learning rate: 0.06
    \item beta: 0.2
    \item sum of excitatory weights: number of nodes / 4
    \item sum of inhibitory weights: number of nodes / 4
\end{itemize}

\newpage

\subsection*{Results of HCP from Bouchard (2015)}

The rule is a modified HCP with possibly different learning rate and regularization term for positive covariance and negative covariance. This could be only applied on excitatory nodes since there is a power law. Here we set $A_{+}=0.05$, $A_{-}=0.06$, and $\beta = 0.2$.

\begin{eqnarray}
    \Delta w_i & = & A_{+} \sigma_i \sigma_o f_{+}(w_i) \text{ if } \sigma_i > 0 \text{ and } \sigma_o > 0 \\
    \Delta w_i & = & A_{-} \sigma_i \sigma_o f_{-}(w_i) \text{ if } \sigma_i \sigma_o < 0
\end{eqnarray}
where 
\begin{eqnarray}
    f_{+}(w_i) & = & (1 - w_i/\sum w_i)^\beta \\
    f_{-}(w_i) & = & (w_i/\sum w_i)^\beta
\end{eqnarray}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0710_structure.png}
\end{figure}

I modify the LQR to make it invariant to the output scale. Why? The rescaling after the output neuron is not "reversible". We can't know advance $y$ should be at what scale.

Normal LQR makes $y$ closed to $\tilde{y}$ (the ideal value before the output neuron) in all aspects. So, the idea is to assume $\tilde{y}$ is of standard scale, that is, having mean of 0 and amplitude of 1. Then, I modify the cost of LQR:

$$V = \sum_{t=t_0+1}^T \bar{y}(t)^T Q_1 \bar{y}(t) + (\frac{y(t) - y_{mean}}{y_{amp}} - \tilde{y}(t))^T Q_2 (\frac{y(t) - y_{mean}}{y_{amp}} - \tilde{y}(t)) + u(t-1)^T R u(t-1)$$ 
where $\bar{y} = \bar{H}y = (I - H^T(HH^{T})^{-1}H)y$, the part of $x$ orthogonal to $H$, $y_{mean}$ is the mean of $y$ in previous 1000 time points, and $y_{amp}$ is the amplitude of $y$ in previous 1000 time points. 

In other words, we standardize $y$ before sending it to the LQR, so that LQR would only correct its period and phase, leaving the scale intact. We implement this idea by augmenting the system.

\newpage

With the input sparse matrix, the activation of neurons is more like a combination of different sine waves rather than a single sine wave.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0710_addshift_ffactivations.png}
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0710_addshift_ffoutputs.png}
\end{figure}

\newpage

I start feedback control at t = 1100, hebbian learning at t = 2000.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0710_addshift_Kris_output.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0710_addshift_Kris_output_Zoom1.png}
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0710_addshift_Kris_output_Zoom2.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0710_addshift_Kris_output_Zoom3.png}
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0710_addshift_FBhebb_output_Zoom4.png}
\end{figure}

\newpage

It seems that feedback control (already invariant tot the scale) can not turn ifself off. Rather, it reaches an equilibrium with Hebbian learning at some non-zero level.

So at t = 7000, I try to turn off feedback control manually by multiplying 1.01 to the R matrix at each time point.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0710_addshift_Kris_unorm.png}
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0710_addshift_Kris_activations.png}
\end{figure}

\newpage

The change of all weights:

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0710_addshift_Kris_weights.png}
\end{figure}

\newpage

With the learned readout weight matrix just before the starting of control decay:

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0710_addshift_Kris_outputs_after.png}
\end{figure}

\newpage

\section*{Week 06/05 - 06/26}

\subsection*{Main Points}

\noindent
\begin{itemize}
    \item Set the pipeline for learning readout weights in RNN.
    \item Add feedback control on shifts, besides gains.
    \item Test only do Hebbian Learning on excitatory nodes.
    \item Implement the Hebbian Covariance Rule from Bouchard (2015).
\end{itemize}
...
\begin{itemize}
    ...
    \item initial gains (recurrent layer): 1
    \item initial shifts (recurrent layer): 0
    \item initial gain (output node): 2.5
    \item initial shift (output node): -2.5
\end{itemize}

\noindent
\textbf{External Inputs}:

A basis set of sine waves with random periods and phases.

\begin{itemize}
    \item periods range: 16 or 20 or 24 
    \item phases range: [0, max phase given period]
\end{itemize}
...
Linear approximation the dynamic system at the current time point: $$x(t+1) = f(x(t), u_g(t), u_s(t)) \approx  Fx(t) + G_gu_g(t) + G_su_s(t) + \text{const} \approx  F_{aug}x(t) + G_{aug}u(t)$$
Then using LQR (discreate-time tracking) on both gains and shifts.
...
\begin{itemize}
    \item rule: Hebbian Covariance Rule (on all nodes / only on excitatory nodes / modified version from Kris)
    \item learning rate: 0.01
    \item negative punishment: 1.5
    \item sum of excitatory weights: number of nodes / 4
    \item sum of inhibitory weights: number of nodes / 4
\end{itemize}
...
During learning, we want the force of Feedback Control and the force of Hebbian Learning to reach a kind of equilibrium: as Hebbian Learning gradually drives the readout weights to the "correct" weights, Feedback Control gradually turns itself off. However, for now there are two things preventing this from happening:

\noindent
\textbf{1. The Scale.} The Hebbian Covariance Rule can not let the output to have a correct mean and amplitude. This scale thing is determined by the sum of weights and the gain and the shift of the output node. If the scale is not correct, the Feedback Control will always be on to correct the scale.

One solution is to adjust the gain and the shift of the output node somehow. For now this adjustment (whether through a mechanism or manually) is added after all learning is done. But this seems to be unnatural.

\noindent
\textbf{2. The Convergence of Hebbian Learning.} As I mentioned in the notes at last meeting, if we want the Feedback Control turns itself off, at a certain time point, when $u_g(t)$ and $u_s(t)$ approaches zero, Hebbian Learning still need to converge on the "correct" weights. This means that Hebbian Learning should converge on the "correct" weights when pre-synaptic activation is the initial activation profile and post-synaptic activation is the target.

One solution is to simply test every possible Hebbian Learning rule on the condition that the initial activation profile and post-synaptic activation is the target. E.g., the tests in Week 05/29 - 06/05. 

\newpage

\subsection*{Results of HCP on All Nodes}

Here we use HCP on all excitatory nodes and inhibitory nodes, with more punishment (1.5) on negative covariance. We hope to give a high weight to the node whose activation has the same phase and period as target. The target here is a sine wave with period of 20.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_FBhebb_output.png}
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_FBhebb_unorm.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_FBhebb_weights_period_neg1.5.png}
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_FBhebb_weights_phase_neg1.5.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_FBhebb_outputs_after.png}
\end{figure}

Mannually adjusting the gain of the output node to 1.5 and the shift of the output node to 0.1: 

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_FBhebb_outputs_after_adjout.png}
\end{figure}

\newpage

\subsection*{Results of HCP on Only Excitatory Nodes}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_Ext_output.png}
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_Ext_unorm.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_Ext_weights_period.png}
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_Ext_weights_phase.png}
\end{figure}

\newpage

Mannually adjusting the gain of the output node to 2.5 and the shift of the output node to 0.15: 

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_Ext_output_after_adjout.png}
\end{figure}
...
The rule is a modified HCP with possibly different learning rate and regularization term for positive covariance and negative covariance. This could be only applied on excitatory nodes since there is a power law. Here we set $A_{+}$ and $A_{-}$ as 0.05 and $\beta = 0.5$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_Kris_output.png}
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_Kris_unorm.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_Kris_weights_period.png}
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_Kris_weights_phase.png}
\end{figure}

\newpage

Mannually adjusting the gain of the output node to 3.2 and the shift of the output node to -1.8: 

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/0626_addshift_Kris_output_after_adjout.png}
\end{figure}

\newpage


\section*{Week 05/29 - 06/05}

\subsection*{Main Points}

\noindent
\begin{itemize}
    \item I investigate deeper to the last week's plan of making the input a set of wave basis.
\end{itemize}

\newpage

\subsection*{Input a Set of Wave Basis (Continue)}

Last week I tried to add a negative punishment (30 times) when using Hebbian Covariance Rule, so that the Hebbian learning could pick out the nodes with the correct period and phase. Kris suggested that I could try some less punishment, e.g., times 1.1.

Here I investigated how the amount of negative punishment would influence the results. The observation is that, with less negative punishment, the nodes with wrong periods or phases would have weights more above 0.

For simplicity, here we assume the feedback control is perfect, so the post synaptic activation of Hebbian learning is always the target (a sine wave with half period 20 and phase 0). The reason of this logic see Week 03/15-03/29.

Here, we let the input to have a random frequency as well as a random phase. $I_i(t) = a_i \sin(\frac{\pi}{p_i}(t+t_i))$, where $p_i$ is a random half period, $t_i$ is a random phase and $a_i$ is a random weight. $p_i$ are randomly chosed from integer values between 15 to 25. The number of nodes is 1000. Half nodes are excitatory and the other half are inhibitory. Sum of excitatory weights is forced to be 250 and sum of inhibitory weights is -250. The whole simulation has 20000 timesteps.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{RNN/FORCE/fig/FORCE_basis_ff_outputs_0605.png}
    \includegraphics[width=0.45\textwidth]{RNN/FORCE/fig/FORCE_basis_ff_activations_0605.png}
\end{figure}

The Hebbian Covariance Rule:
$$h_i \leftarrow h_i + \delta_{punish}(z(t) - \bar{z})\frac{x_i(t) - \bar{x_i}}{\text{std}(x_i)}$$
For excitatory neurons, if $(z(t) - \bar{z})(x_i(t) - \bar{x_i}) < 0$, $\delta_{punish}$ has value above 1, otherwise 1. For inhibitory neurons, if $(z(t) - \bar{z})(x_i(t) - \bar{x_i}) > 0$, $\delta_{punish}$ has value above 1, otherwise 1.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_weights_period_0605_neg1.png}
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_weights_phase_0605_neg1.png}
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_output_0605_neg1.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_weights_period_0605_neg1.1.png}
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_weights_phase_0605_neg1.1.png}
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_output_0605_neg1.1.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_weights_period_0605_neg2.png}
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_weights_phase_0605_neg2.png}
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_output_0605_neg2.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_weights_period_0605_neg5.png}
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_weights_phase_0605_neg5.png}
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_output_0605_neg5.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_weights_period_0605_neg10.png}
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_weights_phase_0605_neg10.png}
    \includegraphics[width=0.9\textwidth]{RNN/FORCE/fig/FORCE_basis_output_0605_neg10.png}
\end{figure}
\newpage

\section*{Week 05/10 - 05/29}

\subsection*{Main Points}

\noindent
\begin{itemize}
    \item I continue to make inputing a set of wave basis work.
\end{itemize}
...
Continuing last week's attempt, we let the input to have a random frequency as well as a random phase. $I_i(t) = a_i \sin(\frac{\pi}{p_i}(t+t_i))$, where $p_i$ is a random half period, $t_i$ is a random phase and $a_i$ is a random weight. Here, $p_i$ are randomly chosed from 3 values: 10, 20, 30. The target signal is a sine wave with half period 20 and phase 0. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{RNN/FORCE/fig/FORCE_basis_ff_outputs_0529.png}
    \includegraphics[width=0.45\textwidth]{RNN/FORCE/fig/FORCE_basis_ff_activations_0529.png}
\end{figure}

We use feedback control to make the output quickly approach the target, and we turn on Hebbian learning from time point 1000. We use the Hebbian Covariance Rule as usual, but one thing is modified: when the covariance is negative, the punishment to the weight is 30 times more than the increment when the covariance is positive. The idea is to make sure that those nodes with wrong frequency and wrong phase would finally have a weight almost equal to 0. This idea is achieved, as shown in the figure below, where only nodes with correct period (40, since the target half period is 20) and correct phase (around 0 and 40) have non-zero weights.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_output_0529.png} \\
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_unorm_0529.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_weights_period_0529.png} \\
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_weights_phase_0529.png}
\end{figure}

Eventually, without feedback control but with the modified readout weight matrix, the output could be closed to the target sine wave, under proper scaling. To imporve the precision, we can add more nodes (now only 200 nodes), so that the phases and periods could distribute more densely and uniformly. With more nodes, it is also possible to add more randomness to the input frequency. But we need more effort to ensure the stability of feedback control and more time to run the simulation.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/FORCE_basis_ff_outputs_after_0529.png}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/FORCE_struc.png}
\end{figure}

The formula for our RNN:
$$x(t+1) = f(x(t), u(t), I(t))$$
$$x(t+1) = (1-c)*x(t) + c*\sigma((g+u(t)) * (Wx(t) + I(t) - s))$$
where $x$ is the activation vector (here 200 nodes), $u$ is the control input vector, $I$ is the input vector, $f$ is non-linear since there is the sigmoid activation function inside, $c$ is the time constant, $\sigma$ is the sigmoid activation function, $g$ is the gain vector, $u$ is applied on the gain (gain modulation), $W$ is the recurrent weight matrix, $s$ is the shift vector. Here, the external input to each node is a sine wave with a random phase but the same frequency as the target, $I_i(t) = a_i \sin(\frac{\pi}{20}(t+t_i))$, where $t_i$ is a random phase and $a_i$ is a random weight. 
...
Where $F = \frac{\partial f}{\partial x}$ and $G = \frac{\partial f}{\partial u}$ at $(x(t), u(t-1), r(t))$. This is like a step of system identification. This step takes in the information of current state $x(t)$, last control input $u(t-1)$, and current external input $r(t)$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{RNN/FORCE/fig/FORCE_ff_outputs_0424.png}
    \includegraphics[width=0.45\textwidth]{RNN/FORCE/fig/FORCE_ff_activations_0424.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/FORCE_hebb_output_0510.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/FORCE_hebb_last_output_0510.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/FORCE_hebb_unorm_0510.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/FORCE_weight_phase_0510.png}
\end{figure}

\newpage

\subsection*{Input a Set of Wave Basis}

In the above section, the frequency of input and target is fixed to be the same. Now, to be more realistic, we let the input to be more random, so that it has a random frequency as well as a random phase. $I_i(t) = a_i \sin(\frac{\pi}{p_i}(t+t_i))$, where $p_i$ is a random half period, $t_i$ is a random phase and $a_i$ is a random weight. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{RNN/FORCE/fig/FORCE_basis_ff_outputs_0510.png}
    \includegraphics[width=0.45\textwidth]{RNN/FORCE/fig/FORCE_basis_ff_activations_0510.png}
\end{figure}

Under this settting, we test feedback control and hebbian learning separately. For feedback control, there is no problem for the output to get to the target.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{RNN/FORCE/fig/FORCE_basis_tracking_output_0510.png}
    \includegraphics[width=0.45\textwidth]{RNN/FORCE/fig/FORCE_basis_tracking_activations_0510.png}
\end{figure}

However, pretending the output is the target, the Hebbian learning (Hebbian Covariance Rule) does not converge to something meaningful. Also, the intuition here is not so clear. Do we expect Hebbian learning to suppress all those nodes with wrong frequencies and phases? Need more investigation.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/FORCE_basis_hebb_output_0510.png}
\end{figure}
...
To my understanding, adaptive control is to deal with the situation when there is a (slow) change in the parameters of the plants. It estimates uncertain plant parameters on-line using measured system signals. It then uses the estimated parameters to compute the control input. Thus, when hebbian learning is doing its work to slowly change the readout weights (or recurrent weights), the controller could change accordingly and maintain a stable performance.

In our framework now, we are directly using the information of the derivative of the non-linear system to linearize it at every time point. Also, we are directly using the information of the full state ($x(t)$). Using adaptive control may give us a more realistic way tf just using the measurement (rather than the full state) to estimate the system and to implement the control. Also, there is example that the adaptive controller could be realized in neural network.

I still need more efforts grasp the techniques of adaptive control. Would that be our next step?

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{RNN/FORCE/fig/MRAC.png}
\end{figure}
