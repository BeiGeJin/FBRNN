...
class RNN:
    ...
    def normal_pdf(self, theta):
        return torch.exp(-0.5 * ((2 * torch.pi * theta)**2))
    ...
    def forward(self, input):
        ...
        self.stim_input = self.input_gaussian(input)
        self.layer_input = torch.matmul(torch.exp(self.log_weight_matrix) * self.connectivity_matrix * self.weight_type, self.activation) + self.stim_input
