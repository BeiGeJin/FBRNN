...
weight_matrix = np.ones((num_nodes, num_nodes)) / 127
...
# Load checkpoint
if LOAD_CHECKPOINT:
    with open('checkpoint/checkpoint_bphebb_comp.pkl', 'rb') as f:
...
for epoch in tqdm(range(start_pos, num_iters), initial=start_pos, total=num_iters, position=0, leave=True):
    ...
    if epoch > 6000 and last_epoch_loss < 0.001 and has_hebbian == False:
        has_hebbian = True
        print("hebbian start!!!")
    ...
    for i in range(ndata):
        ...
        # backprop
        if i > 100 and has_backprop:
            opt = torch.optim.SGD([network.gain, network.shift], lr=backprop_lr)
            # opt = torch.optim.Adam([network.gain, network.shift], lr=backprop_lr)
            loss_val.backward()
            opt.step()
            opt.zero_grad()
        ...
        if i > 100 and has_hebbian:
            ...
            # normalize each column of network.weight_matrix
    ...
    if SAVE_CHECKPOINT and epoch + 1 == checkpoint_epoch:
        with open('checkpoint/checkpoint_bphebb_comp.pkl', 'wb') as f:
...
if not os.path.isdir('../weights/SIN2_bphebb_exp_' + str(num_nodes) + '_nodes'):
    os.mkdir('../weights/SIN2_bphebb_exp_' + str(num_nodes) + '_nodes')
with open('../weights/SIN2_bphebb_exp_' + str(num_nodes) + '_nodes/weight_history.json', 'w') as f:
