...
# Load checkpoint
if LOAD_CHECKPOINT:
    with open('checkpoint/checkpoint_bphcppt.pkl', 'rb') as f:
        init_gain = pickle.load(f)
        init_shift = pickle.load(f)
        weight_matrix = pickle.load(f)
        init_activations = pickle.load(f)
        last_five_activations = pickle.load(f)
        last_activations_deviation = pickle.load(f)
...
for epoch in tqdm(range(start_pos, num_iters), initial=start_pos, total=num_iters, position=0, leave=True):
    ...
    for idx in range(period):
        ...
        # calculate the mean of last_five_activations
        if len(last_five_activations) == 5:
            mean_last_five_activations = torch.mean(torch.cat(last_five_activations, dim=1), dim=1).unsqueeze(1)
            this_activations_deviation = this_activations - mean_last_five_activations

        # update weights by hebbian learning
        if has_hebbian:
            # Calculate Hebbian weight updates
            hebbian_update = torch.matmul(last_activations_deviation, this_activations_deviation.T)
        ...
        if len(last_five_activations) == 5:
            last_activations_deviation = this_activations_deviation.clone() # for hcp
        last_five_activations.append(this_activations.clone())
        if len(last_five_activations) > 5:
            last_five_activations.pop(0)
        ...
        if has_hebbian:
            ...
            activations.append(init_activations)
    ...
    if SAVE_CHECKPOINT and epoch + 1 == checkpoint_epoch:
        with open('checkpoint/checkpoint_bphcppt.pkl', 'wb') as f:
            pickle.dump(init_gain, f)
            pickle.dump(init_shift, f)
            pickle.dump(weight_matrix, f)
            pickle.dump(init_activations, f)
            pickle.dump(last_five_activations, f)
            pickle.dump(last_activations_deviation, f)
...
net_weight_history['activations'] = np.asarray(activations).tolist()
