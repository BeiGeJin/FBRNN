...
for epoch in tqdm(range(start_pos, num_iters), initial=start_pos, total=num_iters, position=0, leave=True):
    ...
    if has_backprop:
        loss, activates = network.train_epoch(targets, time, inputs, learning_rate=0.01, mode='weight')
    # weight boundary
...
if not os.path.isdir('../weights/SIN2_wt_exp_' + str(num_nodes) + '_nodes'):
    os.mkdir('../weights/SIN2_wt_exp_' + str(num_nodes) + '_nodes')
with open('../weights/SIN2_wt_exp_' + str(num_nodes) + '_nodes/weight_history.json', 'w') as f:
