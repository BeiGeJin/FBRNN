...
# print(network.output_weight_matrix)
...
weight_history, losses = network.train(num_iters, targets, time, inputs = inputs,
                                       ...
                                       hebbian_learning = True, reservoir = True,
                                       learning_rate = .005)
...
net_weight_history['output weights'] = np.asarray(weight_history[3]).tolist()
net_weight_history['losses'] = np.asarray(losses).tolist()

if not os.path.isdir('MacheyGlass_liquid_' + str(num_nodes) + '_nodes'):
    os.mkdir('MacheyGlass_liquid_' + str(num_nodes) + '_nodes')
with open('MacheyGlass_liquid_' + str(num_nodes) + '_nodes/weight_history.json', 'w') as f:
